class RDDR::TextBox < RDDR::GTKObject
  ...
  def initialize(text_lines, text_size: 0, frame_alignment: :center, frame_alignment_v: :center, text_alignment: :left, text_offset: 5, frame_offset: 5, max_width: grid.w)
  ...
  def primitives
    ...
    frame.x =
      ...
      when :left   then grid.left.shift_right(@frame_offset)
      when :right  then grid.right.shift_left(@frame_offset + frame.w)
      end
    ...
    frame.y =
      ...
      when :top    then grid.top.shift_down(@frame_offset + frame.h)
      when :bottom then grid.bottom.shift_up(@frame_offset)
      end
